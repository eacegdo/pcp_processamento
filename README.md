...
2. Tabela `public.escola` já existente, com `inep` e as colunas `oce_tipo_acesso`, `oce_status`, `oce_pendencia`.
3. Arquivo `.env` local (copie de `.env.example`):
...
- `escola`: apenas `oce_tipo_acesso`, `oce_status`, `oce_pendencia` via `PATCH` filtrado por `inep` (sem insert/upsert)
